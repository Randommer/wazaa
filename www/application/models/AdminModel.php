...
class AdminModel extends CI_Model
{
    ...
    public function toutesphotos($idAnnonce)
    { //méthode renvoie 
        ...
        //construction de la requête
        $this->db->select("wimg_id as 'id'");
        $this->db->from("waz_images");
        if ($idAnnonce != 0)
        {
            $this->db->where("wimg_wan_id", $idAnnonce);
        }
        $this->db->order_by("wimg_id", 'DESC');
        //envoie de la requête à la base
        $results = $this->db->get();
    ...
    { //méthode renvoie l'objet Annonce dont l'ID est passé en argument
        ...
        //construction de la requête
        $this->db->select("wimg_id as 'id', wimg_ext as 'ext', wan_ref as 'ref', wimg_id as 'file'");
        $this->db->from("waz_images");
        $this->db->join("waz_annonces", "wan_id = wimg_wan_id", "inner");
        $this->db->where("wimg_id", $id);
        //envoie de la requête à la base
        $results = $this->db->get();
    ...
    { //méthode renvoie l'objet Annonce dont l'ID est passé en argument
        ...
        //construction de la requête
        $this->db->select("wan_id as 'id', wan_offre as 'offre', wan_type as 'type', wan_pieces as 'pieces', wan_ref as 'ref', wan_titre as 'titre', wan_description as 'description', wan_local as 'local', wan_surf_hab as 'surf_hab', wan_surf_tot as 'surf_tot', wan_prix as 'prix', wan_diagnostic as 'diagnostic', wan_d_ajout as 'ajout', DATE_FORMAT(wan_d_modif, '%Y-%m-%d %H:%i') as 'modif', wan_vues as 'vues', COUNT(wimg_id) as 'photos'");
        $this->db->from("waz_annonces");
        $this->db->join("waz_images", "wimg_wan_id = wan_id", "left");
